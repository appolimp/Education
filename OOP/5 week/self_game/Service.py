...
class MapFactory(yaml.YAMLObject, ABCMeta):

    @classmethod
    def from_yaml(mcs, loader, node):
        data = loader.construct_mapping(node)
        return {'map': mcs.Map(), 'obj': mcs.Objects()}
...
class EmptyMap(MapFactory):
    ...
    class Objects:
        ...
        def get_objects(self, _map):  # FIXME

            prop = object_list_prob['objects']['stairs']
            for i in range(random.randint(prop['min-count'], prop['max-count'])):
                coord = (random.randint(2, 5), random.randint(2, 5))
                intersect = True
                while intersect:
                    intersect = False
                    if _map[coord[1]][coord[0]] == wall:
