...
class ScreenHandle(pygame.Surface):
    ...
    @property
    def shift(self):   # FIXME
        size = self.engine.sprite_size
        pos = self.engine.hero.position

        shift = [None, None]
        center = [320, 256]

        size_map = [len(self.engine.map[0]), len(self.engine.map)]

        left_bound = [i // size for i in center]

        right_bound = [size_map[i] - center[i]//size for i in [0, 1]]

        for i in [0, 1]:
            if pos[i] < left_bound[i]:
                shift[i] = 0
            elif pos[i] > right_bound[i]:
                shift[i] = right_bound[i] - left_bound[i]
            else:
                shift[i] = pos[i] - left_bound[i]
...
class GameSurface(ScreenHandle):
    ...
    def draw_hero(self):
        self.engine.hero.draw(self, self.shift)
